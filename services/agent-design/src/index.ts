    ...
    try {
      ...
      try {
        ...
        if (evalText.trim().length > 0) {
          const runDocForEval = await getCollection<PipelineRunDoc>(Collections.PIPELINE_RUNS).findOne({ runId: parsed.runId });
          const designTenantId = runDocForEval?.tenantId || (parsed.payload as any)?.tenantId;
          const designPillarId = runDocForEval?.contentPillarId || (parsed.payload as any)?.targetPillarId;
          ...
          const evalRes = await fetch(`${EVALUATOR_URL}/evaluate`, {
            ...
            body: JSON.stringify({
              runId: parsed.runId,
              tenantId: designTenantId,
              pillarId: designPillarId,
              platform: "linkedin",
              text: evalText,
            }),
          });
